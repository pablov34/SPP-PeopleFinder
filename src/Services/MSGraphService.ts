...
export class MSGraphService implements IMSGraphService{
    ...
    public async getUsersByAllProperties(searchFor:string,client:MSGraphClient):Promise<IUserProperties[]>{
        let userProperties:IUserProperties[] = [];
        try {
            let res;
            if(searchFor != null && searchFor.length != 0 && searchFor != "")
            {
                 res = await client.api("users")
                 .version("v1.0")
                 .filter(`(startswith(displayName,'${escape(searchFor)}')) or (startswith(givenName,'${escape(searchFor)}')) or 
                       (startswith(surname,'${escape(searchFor)}')) or (startswith(mail,'${escape(searchFor)}')) or 
                       (startswith(jobTitle,'${escape(searchFor)}'))`)
                 .get();
           }
           else
           {
